...
$MCLANG "$DIR/mc/ballistics.mc" --target rust
...
if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "win32" || "$OSTYPE" == "cygwin" ]]; then
  ...
  gcc -shared -fPIC "$DIR/mc/ballistics.c" -lm -o "$DIR/build/ballistics.so"
fi
...
if [ -n "$EMCC" ]; then
  $EMCC "$DIR/mc/ballistics.c" \
    -o "$DIR/build/ballistics.js" \
    -s EXPORTED_FUNCTIONS='["_range","_max_height","_time_of_flight","_height_at"]' \
