...
echo "=== [1/6] mclang: compile mc/orbital_mechanics.mc ==="
$MCLANG "$DIR/mc/orbital_mechanics.mc"

mkdir -p "$DIR/build"

echo ""
echo "=== [2/6] C: build + run ==="
gcc "$DIR/mc/orbital_mechanics.c" "$DIR/src/c/main.c" -lm -o "$DIR/build/demo"
"$DIR/build/demo"

echo ""
echo "=== [3/6] Python: build shared lib + run ==="
if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "win32" || "$OSTYPE" == "cygwin" ]]; then
  gcc -shared "$DIR/mc/orbital_mechanics.c" -lm -o "$DIR/build/orbital_mechanics.dll"
else
  gcc -shared -fPIC "$DIR/mc/orbital_mechanics.c" -lm -o "$DIR/build/orbital_mechanics.so"
fi
...
if [ -n "$EMCC" ]; then
  $EMCC "$DIR/mc/orbital_mechanics.c" -o "$DIR/build/orbital_mechanics.js" \
    -s EXPORTED_FUNCTIONS='["_vis_viva","_period","_ecc_anomaly","_true_anomaly","_orbit_radius","_orbit_x","_orbit_y","_v_escape","_v_circular","_dv1_hohmann","_dv2_hohmann","_dv_total_hohmann"]' \
...
$MCLANG "$DIR/mc/orbital_mechanics.mc" --target node
...
$MCLANG "$DIR/mc/orbital_mechanics.mc" --target rust
