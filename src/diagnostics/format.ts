import type { Diagnostic, Label } from './diagnostic.js';
...
const red    = (s: string) => ansi('31;1', s);
const yellow = (s: string) => ansi('33;1', s);
const blue   = (s: string) => ansi('34;1', s);
...
export function formatDiagnostic(
  ...
  const allLabels: Label[] = [diag.primary, ...(diag.secondary ?? [])];
  ...
  for (const [file, labels] of byFile) {
    ...
    for (const label of labels) {
      ...
      for (let l = lo; l <= hi; l++) lineSet.add(l);
    }
    ...
    for (const lineNum of linesToShow) {
      ...
      for (const label of labels) {
        if (label.span.start.line !== lineNum) continue;
 ...
 */
export function explainCode(code: string): string | undefined {
