...
export class CGenerator {
    ...
    genTopLevel(node) {
        switch (node.kind) {
            case 'ConstDef':
                this.genConstDef(node);
                break;
            case 'FuncDef':
                this.genFuncDef(node);
                break;
            case 'ImportDef':
                this.emit(`#include "${node.path}.h"`);
                break;
        }
    }
    // ── Const def ───────────────────────────────────────────────────────────────
    genConstDef(node) {
        if (node.value.kind === 'TableExpr') {
            this.genTableDef(node.name, node.value);
            return;
        }
    ...
    genFuncDefSimple(node, isPrivate, cName) {
        const paramStrs = this.buildParamList(node.params);
        const retType = this.funcReturnType(node);
    ...
    genFuncDefWithDefaults(node, isPrivate, cName) {
        ...
        for (let d = 1; d <= defaultCount; d++) {
            ...
            const callArgs = params.map((p, i) => i < provided ? translit(p.name) : this.genExpr(p.default)).join(', ');
    ...
    genIfNode(node) {
        ...
        if (!node.else_) {
            ...
            if (this.isInlineExprBranch(node.then)) {
                const val = this.genExpr(node.then);
        ...
        if (node.else_) {
            if (node.else_?.kind === 'IfNode') {
                this.emit('} else ');
                this.genIfNode(node.else_);
                return;
            }
            this.emit('} else {');
            this.indent++;
            this.genBranch(node.else_);
            this.indent--;
        }
        this.emit('}');
    }
    isInlineExprBranch(branch) {
    ...
    genExpr(expr) {
        switch (expr.kind) {
            ...
            case 'StringLitExpr': return `"${expr.value}"`;
    ...
    genNumber(expr) {
        ...
        if (!isFinite(expr.value))
            return expr.value > 0 ? 'INFINITY' : '-INFINITY';
    ...
    inferType(expr) {
        if (expr.kind === 'IdentExpr')
            return this.typeEnv.get(expr.name);
        return undefined;
    }
    inferLenExpr(expr) {
        if (expr.kind === 'IdentExpr') {
    ...
    genBinary(expr) {
        if (expr.op === '⋅')
            return this.genDot(expr);
        if (expr.op === '⨯')
            return this.genCross(expr);
...
export function generateC(ast, opts) {
