...
import { CompilerError, formatDiagnostic, explainCode, } from '../diagnostics/index.js';
const HELP = `
    ...
    try {
        ...
        if (dumpTokens) {
            for (const tok of tokens) {
                ...
                console.log(`${String(kind).padEnd(16)} ${JSON.stringify(value).padEnd(20)} ` +
                    `${span.start.line}:${span.start.col}–${span.end.line}:${span.end.col}`);
            }
            return;
        }
        const hasImports = tokens.some(t => t.kind === "import" /* TokenKind.KwImport */);
...
function genPythonStub(base, _h) {
