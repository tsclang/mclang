...
```
Error E033: circular dependency in where block: a → b → a
```

### Теневое переопределение встроенной константы

Если имя переменной совпадает с именем встроенной константы (`π`, `e`, `τ`, `φ`), компилятор выдаёт предупреждение:

```mc
f(x) =
    π * x
    where
        π = 10   // предупреждение: shadowing standard constant 'π'
```

Встроенная константа перекрывается в рамках этой функции. Чтобы избежать путаницы, используй другое имя.
